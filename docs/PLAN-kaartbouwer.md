...
- **Standaardgedrag, geen keuze:** het lakzegel kraakt en valt in twee helften weg, dan opent de klep, dan wordt de kaart uit de envelop getrokken en landt hij op zijn plek. Dit is niet beter of slechter naar smaak, het is simpelweg beter, dus hier hoort geen knop bij.
- **Waarom dit niet met CSS-keyframes kan.** De eerste poging (11 sep 2026) ging te snel en het leek niet alsof de kaart uit de envelop kwam. Drie redenen, alle drie fout in de opzet en niet in de tijdsduur:
  ...
  3. De envelop was één laag, dus de kaart lag erachter in plaats van erin. Nu bestaat de envelop uit drie lagen rond de kaart, met de vorm van een echte envelop:
     ...
     - **kaart** (2).
     - **klep** (4 zolang hij vóór de kaart hoort, 1 daarna): precies de driehoek die de voorkant openlaat. Dicht is hij de buitenkant van de envelop en ligt hij dus vóór de kaart; open klapt hij naar achteren en gaat de kaart er juist vóór langs. Die wissel is één z-index. Hij wordt ook donkerder bij het openen: je ziet dan de binnenkant, en dat maakt hem zichtbaar tegen een achtergrond met bijna dezelfde kleur.
     ...
     De laag om die lagen heen mag geen `transform` en geen `z-index` hebben, want dat maakt een eigen stapelcontext en dan kan de kaart er niet meer tussen; het zweven staat daarom op de drie lagen zelf (op `transform`, want `top` animeren betekent elke frame layout en dat schokt) en die lagen krijgen elk apart hun transform per frame.
...
- **Hoe het nu werkt.** De kaart ligt vanaf het begin in de envelop, met zijn bovenrand op 14 procent van de envelophoogte, dus zodra de klep opengaat zie je hem er al in liggen. De voorkant dekt hem daarna zelf af, dus de klip op de kaart hoeft alleen te verbergen wat ónder de envelop uitkomt. De kaart komt omhoog en de envelop zakt precies zo ver dat zijn bovenrand onder de onderkant van de kaart uitkomt; pas dan dekt hij niets meer af en is er nergens gesmokkeld. De beweging is puur vloeiend: een veertje aan het eind schoot de kaart nog een paar pixels omhoog en dat leest als een hapering. Dat rekenen gebeurt in een `requestAnimationFrame`-lus in `card-reveal.tsx`, niet in keyframes: de hoogtes zijn pas bij het openen bekend en moeten worden opgemeten.
...
- **Nog vier details uit de testronde van 11 september:**
  1. De dichte envelop zweeft op en neer, en op het hoogste punt kwam er een randje kaart onder de envelop uit. De klip trekt nu `ENVELOP_ZWEEF` van de onderrand af, dus op het hoogste punt sluit het precies aan. Dat zweven liep eerst op `top`, wat elke frame layout betekent en dus schokt; het loopt nu op `transform` en staat op de drie lagen zelf, niet op de laag eromheen (die mag geen transform hebben). De uitslag is 4 pixels, zodat het niet opvalt als de animatie stopt en het rekenwerk de transform overneemt.
  2. De afronding van de kaart las als vierkant, en dat was de schaduw. Een wijde schaduw krult om de hoeken heen en maakt de uitsparing naast de afronding donkerder dan de kaart zelf; omdat de achtergrond in dit thema bijna dezelfde kleur heeft, lijkt de kaart dan vierkant door te lopen. De schaduw heeft nu een negatieve spread (`0 26px 50px -26px`), dus hij valt alleen onder de kaart, plus een heel dun lichtrandje dat de afronding volgt. Buiten de rand is het nu lichter dan de kaart in plaats van donkerder.
  3. Je zag de kaart al voordat de klep openging, omdat de klep bij het begin van het draaien al achter de kaart ging. Dat gebeurt nu halverwege het draaien (`DUUR_KLEP / 2`), want daar staat hij recht overeind. Daarvoor ligt hij ervoor.
  4. Het beeld verschoof een paar pixels tijdens het uitschuiven, omdat de zakkende envelop de pagina hoger maakte (822 naar 864 pixels) en er soms een scrollbalk bij kwam. De buitenste doos heeft nu `overflow: clip`, dus de scrollhoogte blijft gelijk. Clip en niet hidden, zodat het geen scrollcontainer wordt.
  5. De kaart versprong nog een laatste keer precies als hij stil hoorde te staan. Dat zat niet in de animatie maar in de indeling: op dat moment verschijnen de knoppen, de CTA en de afzenderregel onder de kaart, waardoor de pagina hoger wordt en de flexbox opnieuw centreert. Die blokken nemen hun plek nu vanaf het begin in (`eindBlok`: `visibility: hidden` plus geen muisgebeurtenissen) en worden alleen zichtbaar gemaakt. Gemeten: de wrapper blijft 629 pixels in rust, tijdens en na de animatie.
