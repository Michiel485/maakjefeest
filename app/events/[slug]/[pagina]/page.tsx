...
export default async function EventSubPage({
  ...
  return (
    <div style={{ maxWidth: 672, margin: "0 auto", padding: "52px 24px 96px" }}>
...
function PageContent({ page, sc }: { page: Page; sc: SC }) {
  ...
  if (page.type === "programma") {
    ...
    return (
      <div style={{ border: `1px solid ${sc.accent}20`, borderRadius: 16, overflow: "hidden" }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: "flex", gap: 16, padding: "16px 24px", backgroundColor: i % 2 === 0 ? sc.navBg : `${sc.accent}06` }}>
            <span style={{ fontSize: "0.875rem", fontWeight: 700, width: 56, flexShrink: 0, paddingTop: 2, color: sc.labelColor }}>{item.time}</span>
  ...
  if (page.type === "praktisch") {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 16 }}>
        {items.map((item, i) => (
          <div key={i} style={{ borderRadius: 16, border: `1px solid ${sc.accent}20`, backgroundColor: `${sc.accent}06`, padding: 20 }}>
            <p style={{ fontSize: "0.6875rem", fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.1em", marginBottom: 4, color: sc.labelColor }}>{item.label}</p>
            <p style={{ fontWeight: 600, color: sc.headingColor, margin: 0 }}>{item.value}</p>
          </div>
        ))}
      </div>
    )
  ...
  if (page.type === "wishlist") {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 16 }}>
        {items.map((item, i) => (
          <div key={i} style={{ borderRadius: 16, border: `1px solid ${sc.accent}20`, padding: 20, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
            <div>
              <p style={{ fontWeight: 600, fontSize: "0.9375rem", color: sc.headingColor, margin: 0 }}>{item.name}</p>
              {item.price && <p style={{ fontSize: "0.8125rem", color: sc.bodyText, margin: "2px 0 0" }}>{item.price}</p>}
            </div>
            {item.url && (
...
function Placeholder({ children, sc }: { children: React.ReactNode; sc: SC }) {
  return (
    <div style={{ borderRadius: 16, border: `1px solid ${sc.accent}20`, backgroundColor: `${sc.accent}06`, padding: "20px 24px", fontSize: "0.9375rem", fontStyle: "italic", color: sc.bodyText }}>
      {children}
    </div>
  )
}
