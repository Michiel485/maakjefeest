...
export async function GET(
  ...
  const { data: event } = await service
    .from("events")
    .select("id, slug, type, title, concept_naam, hoort_bij, datum, locatie, style, font_hero, font_initials, font_frame_names, font_page_titles, hero_image_url, hero_overlay, nav_layout, nav_title, use_frame, frame_style, initials, frame_names, frame_location, frame_initials_size, frame_names_size, frame_date_size, frame_location_size, hero_image_pos_x, hero_image_pos_y, status, plan, homepage_settings, pw_enabled, pw_type, pw_value, pw_question, pw_answer")
    .eq("id", event_id)
